...
class LcmFoxgloveBridgeRunner:
    ...
    async def _process_topic_queue(self):
        ...
        while True:
            try:
                ...
                if self.server:
                    try:
                        # Convert schema type from package.MsgType to package/msg/MsgType format for Foxglove
                        ...
                        print(f"  Registered Foxglove channel: {topic_info.name} with schema: {foxglove_schema_name}")
                    except Exception as e:
            ...
            try:
                ...
                if self.server and topic_info and topic_info.channel_id:
                    try:
                        ...
                        if self.debug:
                            print(f"Sending message on channel {topic_info.name} with timestamp {timestamp_ns}: {json_data[:200]}...")
    ...
    def _format_image_msg(self, msg, topic_info=None):
        ...
        try:
            ...
            if self.debug:
                print(f"Image data length: {data_length}, height: {msg.height}, width: {msg.width}, encoding: {msg.encoding}")
            ...
            if hasattr(msg, "data") and data_length > 0:
                ...
                if self.debug:
                    print(f"Image raw data length: {len(image_data_bytes)} bytes")
                    print(f"First 20 bytes: {[b for b in image_data_bytes[:20]]}")
            ...
            # Get encoding value with a default
            encoding = msg.encoding if hasattr(msg, "encoding") and msg.encoding else "rgb8"
            
            # Return properly formatted message dict - CRITICAL for Foxglove Image panel
            return {
                "header": header,
                "height": msg.height,
                "width": msg.width,
    ...
    def _format_compressed_image_msg(self, msg, topic_info=None):
        ...
        try:
            ...
            if self.debug:
                print(f"CompressedImage format: {image_format}, data length: {data_length}")
            ...
            if hasattr(msg, "data") and data_length > 0:
                ...
                # For debugging
                if self.debug:
                    print(f"CompressedImage raw data length: {len(image_data_bytes)} bytes")
                    print(f"First 20 bytes: {[b for b in image_data_bytes[:20]]}")
            ...
            # Return properly formatted message - CRITICAL for Foxglove Image panel
            return {
                "header": header, 
                "format": image_format,
                "data": image_data
    ...
    def _format_tf_msg(self, msg, topic_info=None):
        ...
        try:
            ...
            if self.debug:
                print(f"Processing TF message with {transforms_length} transforms")
                
                # Print some detailed info about the TF message
                print(f"TF message attributes: {[attr for attr in dir(msg) if not attr.startswith('_') and not callable(getattr(msg, attr))]}")
                if hasattr(msg, "transforms"):
                    print(f"TF message has {len(msg.transforms)} transforms in the list")
            ...
            if hasattr(msg, "transforms") and transforms_length > 0:
                for i in range(min(transforms_length, len(msg.transforms))):
                    ...
                    if transform_dict:
                        ...
                        if self.debug:
                            print(f"Transform {i}: {transform_dict}")
            
            # Foxglove expects the TF message to have a transforms array matching /tf2_msgs/msg/TFMessage
            # The exact format is critical - see test_foxglove.py in the foxglove-websocket-python repo
            ...
            if self.debug:
                ...
                if hasattr(transform, "transform"):
                    ...
                    if hasattr(t, "translation"):
                        print(f"Translation: ({getattr(t.translation, 'x', 0)}, {getattr(t.translation, 'y', 0)}, {getattr(t.translation, 'z', 0)})")
                    if hasattr(t, "rotation"):
                        print(f"Rotation: ({getattr(t.rotation, 'x', 0)}, {getattr(t.rotation, 'y', 0)}, {getattr(t.rotation, 'z', 0)}, {getattr(t.rotation, 'w', 1)})")
            
            # Check for required attributes
            if not hasattr(transform, "transform"):
                print("Warning: TransformStamped missing 'transform' attribute")
                return None
                
            if not hasattr(transform.transform, "translation") or not hasattr(transform.transform, "rotation"):
                print("Warning: Transform missing translation or rotation")
                return None
