...
class SchemaGenerator:
    ...
    def generate_schema(self, schema_type):
        ...
        if schema_type in HARDCODED_SCHEMAS:
            print(f"Using hardcoded schema for {schema_type}")
            ...
            try:
                ...
                if self.server and topic_info and topic_info.channel_id:
                    try:
                        ...
                        if self.debug:
                            print(f"Sending message on channel {topic_info.name} with timestamp {timestamp_ns}: {json_data[:100]}...")
    ...
    def _format_image_msg(self, msg, topic_info=None):
        ...
        try:
            ...
            if encoding.lower() == "bgr8":
                encoding = "rgb8"  # Try using rgb8 if Foxglove doesn't recognize bgr8
            
            if self.debug:
                print(f"Image data length: {data_length}, height: {msg.height}, width: {msg.width}, encoding: {encoding}")
        ...
        try:
            ...
            if self.debug:
                ...
                if hasattr(msg, "transforms") and transforms_length > 0 and len(msg.transforms) > 0:
                    ...
                    print(f"First transform child frame: {tf_sample.child_frame_id if hasattr(tf_sample, 'child_frame_id') else 'unknown'}")
                ...
                for i in range(min(transforms_length, len(msg.transforms))):
                    ...
                    if transform_dict:
                        ...
                        if self.debug and i == 0:
                            ...
                            print(f"First transform: {json.dumps(transform_dict)[:200]}...")
