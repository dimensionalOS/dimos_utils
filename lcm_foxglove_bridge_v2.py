            ...
            if channel not in self.topics:
                ...
                if "#" in channel:
                    ...
                    try:
                        ...
                        print(f"Error processing mapped topic {channel} with schema {schema_type}: {e}")

class LcmFoxgloveBridgeRunner:
    """Runner class to manage the bridge and server lifecycle"""
    def __init__(self, host="0.0.0.0", port=8765, schema_map=None):
        self.host = host
        self.port = port
    ...
    async def _process_topic_queue(self):
        ...
        while True:
            try:
                ...
                if self.server:
                    try:
                        # Special handling for Image and TF topics to ensure correct schema display
                        channel_info = {
    ...
    def _on_lcm_message(self, channel, data):
        ...
        if not topic_info or not self.loop:
            print(f"Warning: Received message for unknown channel: {channel}")
            return
        ...
        try:
            ...
            if topic_info.lcm_class:
                try:
                    ...
                    msg_dict = self._lcm_to_dict(msg, topic_info)
                    print(f"Received message on {channel} of type {topic_info.schema_type}")
                    # Print message structure for debugging (shortened for readability)
                ...
                print(f"Received message on {channel} but no decoder available, using raw data")
                msg_dict = {
                    "_raw_data": base64.b64encode(data).decode("ascii")
                }
    ...
    def _lcm_to_dict(self, msg, topic_info=None):
        """Convert an LCM message to a dictionary with special handling for certain types"""
        if isinstance(msg, (int, float, bool, str, type(None))):
            ...
            if topic_info and topic_info.schema_type == "sensor_msgs.Image":
                return self._format_image_msg(msg)
            elif topic_info and topic_info.schema_type == "sensor_msgs.CompressedImage":
                return self._format_compressed_image_msg(msg)
            elif topic_info and topic_info.schema_type == "tf2_msgs.TFMessage":
                return self._format_tf_msg(msg)
            elif topic_info and topic_info.schema_type == "sensor_msgs.JointState":
                return self._format_joint_state_msg(msg)
            
            # For all other message types, use the general approach
            result = {}
            ...
            for attr, info in attribute_info.items():
                ...
                try:
                    ...
                    print(f"Error converting attribute {attr}: {e}")
                    result[attr] = None
                    
            return result
    
    def _format_image_msg(self, msg):
        """Format an Image message for Foxglove"""
        try:
            # Format the header
            header = self._get_header_dict(msg.header)
            
            # For Image messages, we need to encode the data as base64
            image_data = base64.b64encode(msg.data[:msg.data_length] if hasattr(msg, 'data_length') else msg.data).decode("ascii")
            
            # Create a properly formatted Image message for Foxglove
            return {
                "header": header,
                "height": msg.height,
                "width": msg.width,
            ...
            print(f"Error formatting Image message: {e}")
            return {"error": str(e)}
    
    def _format_compressed_image_msg(self, msg):
        """Format a CompressedImage message for Foxglove"""
        try:
            # Format the header
            header = self._get_header_dict(msg.header)
            
            # For CompressedImage messages, we need to encode the data as base64
            image_data = base64.b64encode(msg.data[:msg.data_length] if hasattr(msg, 'data_length') else msg.data).decode("ascii")
            
            # Create a properly formatted CompressedImage message for Foxglove
            return {
                "header": header,
                "format": msg.format,
                "data": image_data
            ...
            print(f"Error formatting CompressedImage message: {e}")
            return {"error": str(e)}
    
    def _format_tf_msg(self, msg):
        """Format a TFMessage for Foxglove"""
        try:
            ...
            if hasattr(msg, "transforms"):
                for i in range(transforms_length):
                    if i < len(msg.transforms):
                        transform = msg.transforms[i]
                        transforms.append(self._format_transform_stamped(transform))
            
            return {"transforms": transforms}
        except Exception as e:
            print(f"Error formatting TFMessage: {e}")
            return {"transforms": []}
    
    def _format_transform_stamped(self, transform):
        """Format a TransformStamped message for Foxglove"""
        try:
            # Format the header
            ...
            tf = {
                "translation": {
                    "x": getattr(transform.transform.translation, "x", 0.0),
                    "y": getattr(transform.transform.translation, "y", 0.0),
                    "z": getattr(transform.transform.translation, "z", 0.0)
                },
                "rotation": {
                    "x": getattr(transform.transform.rotation, "x", 0.0),
                    "y": getattr(transform.transform.rotation, "y", 0.0),
                    "z": getattr(transform.transform.rotation, "z", 0.0),
                    "w": getattr(transform.transform.rotation, "w", 1.0)
                }
            }
            ...
            print(f"Error formatting TransformStamped: {e}")
            return {}
    
    def _format_joint_state_msg(self, msg):
        """Format a JointState message for Foxglove"""
        try:
            # Format the header
            header = self._get_header_dict(msg.header)
            ...
            return {
                "header": header,
                "name": msg.name[:name_length] if hasattr(msg, "name") else [],
                "position": msg.position[:position_length] if hasattr(msg, "position") else [],
                "velocity": msg.velocity[:velocity_length] if hasattr(msg, "velocity") else [],
                "effort": msg.effort[:effort_length] if hasattr(msg, "effort") else []
            }
        except Exception as e:
            print(f"Error formatting JointState: {e}")
            return {"header": header, "name": [], "position": [], "velocity": [], "effort": []}
        ...
        try:
            ...
            if hasattr(header, "stamp") and not isinstance(header.stamp, (int, float)):
                if hasattr(header.stamp, "sec") and hasattr(header.stamp, "nsec"):
                    stamp = {
                        "sec": header.stamp.sec if hasattr(header.stamp, "sec") else 0,
                        "nanosec": header.stamp.nsec if hasattr(header.stamp, "nsec") else 0
                    }
                else:
...
async def main():
    ...
    if args.map_file:
        try:
            ...
            print(f"Loaded schema map from {args.map_file} with {len(schema_map)} entries")
        except Exception as e:
            print(f"Error loading schema map file: {e}")
    
    # Create and run the bridge
    bridge = LcmFoxgloveBridgeRunner(host=args.host, port=args.port, schema_map=schema_map)
    await bridge.run()
