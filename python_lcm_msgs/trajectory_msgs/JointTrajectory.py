...
class JointTrajectory(object):
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != JointTrajectory._get_packed_fingerprint():
            raise ValueError("Decode error")
        return JointTrajectory._decode_one(buf)
    ...
    def _get_hash_recursive(parents):
        if JointTrajectory in parents: return 0
        newparents = parents + [JointTrajectory]
    ...
    def _get_packed_fingerprint():
        if JointTrajectory._packed_fingerprint is None:
            JointTrajectory._packed_fingerprint = struct.pack(">Q", JointTrajectory._get_hash_recursive([]))
        return JointTrajectory._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", JointTrajectory._get_packed_fingerprint())[0]
