...
class LCMBagRecorder:
    ...
    def __init__(self, output_path: str, lcm_url: str = DEFAULT_LCM_URL, 
        ...
        Args:
            ...
            json_encoding: If True, create an additional MCAP file with JSON encoding for Foxglove compatibility
        """
        ...
        self.lc = None
        self.writer = None
        self.json_writer = None  # Writer for JSON encoded messages
        self.subscription = None
        self.record_thread = None
        self.write_thread = None
        self.channel_schema_ids: Dict[str, int] = {}
        self.json_channel_schema_ids: Dict[str, int] = {}  # Schema IDs for JSON encoded channels
    ...
    def _get_chunk_filename(self, chunk_num=None, json_format=False):
        """
        Get filename for a specific chunk.
        
        Args:
            chunk_num: Chunk number (None for current chunk)
            json_format: If True, return the JSON format filename
        """
        if chunk_num is None:
            chunk_num = self.current_chunk
        
        # Always split the filename base and extension    
        base, ext = os.path.splitext(self.output_path)
        
        # Construct the filename based on format and chunking
        if self.max_file_size_mb <= 0:
            # No chunking
            if json_format:
                return f"{base}_foxglove{ext}"
            else:
                return f"{base}_lcm{ext}"
        else:
            # With chunking
            if json_format:
                return f"{base}_chunk{chunk_num}_foxglove{ext}"
            else:
                return f"{base}_chunk{chunk_num}_lcm{ext}"
    ...
    def _create_mcap_writer(self, filename=None):
        ...
        try:
            ...
            logger.info(f"Created MCAP writer for {filename} with LCM encoding")
            
            # Create a JSON format writer if enabled
            if self.json_encoding:
                json_filename = self._get_chunk_filename(json_format=True)
                self.json_writer = MCAPWriter(open(json_filename, 'wb'))
                
                # For Foxglove compatibility, use minimal profile options
                self.json_writer.start(
                    profile="",
                    library="lcmbag-foxglove",
                )
                
                # Configure writer to use a specific chunk size for better reliability
                # Also ensures deterministic timestamp handling
                self.json_writer.set_chunk_size(512 * 1024)  # 512KB chunks
                
                logger.info(f"Created JSON MCAP writer for {json_filename} for Foxglove compatibility")
        except Exception as e:
            logger.error(f"Failed to create MCAP writer: {e}")
            raise
    
    def _lcm_to_json(self, channel: str, data: bytes, message_type: str) -> bytes:
        """
        Convert an LCM message to JSON format for Foxglove compatibility.
        
        Args:
            channel: The channel name
            data: The raw LCM message data
            message_type: The message type string (e.g., "geometry_msgs/Twist")
            
        Returns:
            JSON-encoded message data as bytes
        """
        try:
            # Handle different message types
            if message_type == "geometry_msgs/Vector3":
            ...
            elif message_type == "geometry_msgs/Point":
                from geometry_msgs import Point
                msg = Point.decode(data)
                json_data = {
                    "x": msg.x,
                    "y": msg.y,
                    "z": msg.z
                }
            elif message_type == "geometry_msgs/Quaternion":
                from geometry_msgs import Quaternion
                msg = Quaternion.decode(data)
                json_data = {
                    "x": msg.x,
                    "y": msg.y,
                    "z": msg.z,
                ...
                # For images, we need to convert the binary data to base64
                import base64
                json_data = {
                    "header": {
                        "stamp": {
                            "sec": msg.header.stamp // 1000000000,
                            "nanosec": msg.header.stamp % 1000000000
                        },
            ...
            elif message_type == "sensor_msgs/CompressedImage":
                from sensor_msgs import CompressedImage
                msg = CompressedImage.decode(data)
                import base64
                json_data = {
                    "header": {
                        "stamp": {
                ...
                # Default handling for unknown types - store raw data in base64
                import base64
                json_data = {
                    "type": message_type,
            ...
            # For Foxglove compatibility, ensure we include key fields
            # For visualization in Foxglove, make sure header and frame_id exist
            if "header" not in json_data and message_type not in ["geometry_msgs/Vector3", "geometry_msgs/Point", "geometry_msgs/Quaternion"]:
                # Simple pseudo-header to help with visualization
                json_data["header"] = {
                    "stamp": {
                        "sec": int(time.time()),
                        "nanosec": 0
            ...
            # Add frame_id for 3D visualization if not already present
            if "frame_id" not in json_data and "header" not in json_data:
                json_data["frame_id"] = "base_link"
            ...
            # Return a valid but minimal JSON object if conversion fails
            return b'{"error": "Failed to convert message"}'
    ...
    def _on_message(self, channel, data):
        ...
        # Initialize channel IDs to None
        channel_id = None
        ...
        if self.recording_mode == self.MODE_NORMAL or self.writer:
            ...
            # Register channel for JSON format if JSON encoding is enabled
            if self.json_encoding and self.json_writer:
                if channel in self.json_channel_schema_ids:
                    json_channel_id = self.json_channel_schema_ids[channel]
                else:
                    try:
                        # Simple approach: use a minimal schema that works with Foxglove
                        # Create a simple schema definition for Foxglove compatibility
                        # Convert message type from ROS1 (geometry_msgs/Twist) to ROS2 (geometry_msgs.msg.Twist) format
                        ros2_type = message_type.replace('/', '.msg.')
                        json_schema = {
                            "type": "object", 
                            "name": ros2_type,
                            "description": ros2_type
                        }
                        # Try to use a format that's explicitly recognized by Foxglove
                        schema_name = "foxglove::"+ros2_type
                        schema_text = json.dumps(json_schema)
                        
                        json_schema_id = self.json_writer.register_schema(
                            name=schema_name,
                            encoding="jsonschema",
                            data=schema_text.encode('utf-8')
                        )
                        
                        # Register channel with correct encoding for Foxglove
                        json_channel_id = self.json_writer.register_channel(
                            topic=channel,
                            schema_id=json_schema_id,
                            message_encoding="json",
                        )
                        
                        self.json_channel_schema_ids[channel] = json_channel_id
                        logger.info(f"Registered JSON channel {channel} with type {message_type}")
                        
                    except Exception as e:
                        logger.error(f"Failed to register JSON channel {channel}: {e}")
                        # Continue even if JSON registration fails
        
        # Create message object with all necessary information
        message_obj = {
            "channel": channel,
            "channel_id": channel_id,
            "json_channel_id": json_channel_id,
            "timestamp": timestamp_ns,  # Using adjusted timestamp!
            "data": data,
            "real_time": int(current_time * 1e9),  # Store real time for debugging
            "message_type": message_type  # Store message type for later channel registration
        }
    ...
    def _write_messages_to_file(self, messages_to_write):
        ...
        for msg in messages_to_write:
            try:
                ...
                json_channel_id = msg.get("json_channel_id", None)
                ...
                # Register JSON channel if needed
                if self.json_encoding and self.json_writer and channel not in self.json_channel_schema_ids:
                    try:
                        # Simple approach: use a minimal schema that works with Foxglove
                        # Create a simple schema definition for Foxglove compatibility
                        # Convert message type from ROS1 (geometry_msgs/Twist) to ROS2 (geometry_msgs.msg.Twist) format
                ...
                # Get adjusted timestamp for both writers
                ...
                # Write to JSON format file if enabled
                if self.json_encoding and self.json_writer:
                    # Get or fetch the JSON channel ID
                    if json_channel_id is None:
                        json_channel_id = self.json_channel_schema_ids.get(channel)
                    
                    if json_channel_id is not None:
                        try:
                            # Convert message to JSON format
                            json_data = self._lcm_to_json(channel, msg["data"], message_type)
                            
                            # Only write if we have valid JSON data (non-empty)
                            if json_data and len(json_data) > 2:  # More than just '{}'
                                # Make sure both timestamps are exactly the same to prevent Foxglove errors
                                exact_timestamp = adjusted_time
                                self.json_writer.add_message(
                                    channel_id=json_channel_id,
                                    log_time=exact_timestamp,
                                    publish_time=exact_timestamp,
                                    data=json_data
                                )
                        except Exception as e:
                            logger.error(f"Error writing JSON message for {channel}: {e}")
            except Exception as e:
    ...
    def _rotate_file(self):
        ...
        try:
            self.writer.finish()
            logger.info(f"Finished LCM writer for chunk {self.current_chunk}")
            
            if self.json_encoding and self.json_writer:
                self.json_writer.finish()
                logger.info(f"Finished JSON writer for chunk {self.current_chunk}")
        except Exception as e:
            logger.error(f"Error finishing writer(s): {e}")
        ...
        self.json_channel_schema_ids = {}  # Reset JSON schema IDs for new file
    ...
    def stop(self):
        ...
        if self.message_buffer:
            ...
            logger.info(f"Writing remaining {message_count} messages")
            ...
            if self.writer:
                try:
                    ...
                    logger.error(f"Error writing remaining messages to LCM file: {e}")
            
            # For JSON format
            if self.json_encoding and self.json_writer:
                try:
                    for msg in self.message_buffer:
                        json_channel_id = msg.get("json_channel_id")
                        if json_channel_id is None:
                            json_channel_id = self.json_channel_schema_ids.get(msg["channel"])
                            
                        if json_channel_id is not None:
                            try:
                                # Convert message to JSON format
                                json_data = self._lcm_to_json(
                                    msg["channel"], 
                                    msg["data"], 
                                    msg["message_type"]
                                )
                                ...
                                logger.error(f"Error writing JSON message during cleanup: {e}")
                except Exception as e:
                    logger.error(f"Error writing remaining messages to JSON file: {e}")
        ...
        # Finalize the MCAP files
        if self.writer:
            try:
                self.writer.finish()
                logger.info(f"Finalized LCM format MCAP file: {self._get_chunk_filename()}")
            except Exception as e:
                logger.error(f"Error finishing LCM writer: {e}")
            finally:
                self.writer = None
        ...
        if self.json_encoding and self.json_writer:
            try:
                self.json_writer.finish()
                logger.info(f"Finalized JSON format MCAP file: {self._get_chunk_filename(json_format=True)}")
            except Exception as e:
                logger.error(f"Error finishing JSON writer: {e}")
            finally:
                self.json_writer = None
...
def main():
    ...
    record_parser.add_argument('--json', action='store_true',
                              help='Create a parallel MCAP file with JSON encoding for Foxglove compatibility')
